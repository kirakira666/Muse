...
import 'package:muse/show/idea_detail.dart';
import 'package:muse/theme/pic_app_theme.dart';
import 'glass_view.dart';
var context1;
class SingleIdeaView extends StatelessWidget {
  ...
  const SingleIdeaView({Key? key, required this.animationController, required this.animation, required this.nameJJ, required this.username, required this.picUrl, required this.urlList, required this.like, required this.id})
      : super(key: key);
  ...
  DecorationImage kkimage() {
    if (picUrl[0] != 'u'){
      return new DecorationImage(
        //设置背景图片
        image: AssetImage("assets/image/star.GIF"),
        fit: BoxFit.cover,
      );
    }else{
      return new DecorationImage(
        //设置背景图片
        image:NetworkImage(cloudInfo.cloudUrlHttp+picUrl),
        fit: BoxFit.cover,
      );
    }
  }
